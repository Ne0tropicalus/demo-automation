# Get the username and password from the SQL Credential
	...
	$stage_tbl_name = Get-AutomationVariable -Name 'REPAIR_ORDERS_STG'
	$external_tbl_name = Get-AutomationVariable -Name 'REPAIR_ORDERS_RAW'
	$provider = Get-AutomationVariable -Name 'Partner B Storage Account'
...
$now=(get-date -format "yyyy-MM-dd HH:mm") +" Running Repair Orders Stored Procedure"
write-output $now

$Cmd=new-object system.Data.SqlClient.SqlCommand("EXEC [TCDEVSQLDW].[dbo].[repair_order_ingest]
													@provider = N'$provider',
													@file_compressed = N'Y',
